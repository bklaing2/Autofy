...
SORT = {
    ...
    'ARTIST': 'by-artist',
    'RELEASE': 'old-to-new',
    'RELEASE_REVERSE': 'new-to-old'
}

UPDATE = {
    'ARTIST_POSTS': 'when-artist-posts',
    'USER_FOLLOWS': 'when-i-follow-artist',
    'USER_UNFOLLOWS': 'when-i-unfollow-artist'
}
...
def get_albums_by_artist(artist_id, spotify):
    albums = []
    offset = 0
...
def get_tracks_in_album(album_id, spotify):
    tracks = []
    offset = 0
    while True:
        results = spotify.album_tracks(album_id, offset=offset)['items']
...
def get_tracks(artist_ids, spotify):
    albums = tracks = []
    for artist_id in artist_ids:
        albums.extend(get_albums_by_artist(artist_id, spotify))

    for album_id in get_ids(albums):
        tracks.extend(get_tracks_in_album(album_id, spotify))
...
def create_playlist(spotify, sort_tracks=None):
    # Get all artists, albums, and tracks
    artists = get_followed_artists(spotify)
    ...
    all_tracks = get_albums_tracks(albums, spotify)

    if sort_tracks == SORT['ALPHA']:
        all_tracks = sorted(all_tracks, key=lambda track: track['name'].casefold())

    print_names(all_tracks)
